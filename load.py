...
def load_data(path,device ='cuda'):
    df = pd.read_csv(f"{path}",encoding='latin')
    #print(df.head())
...
def preproc_data(path,block_size=128,batch_size=64,device='cuda'):
    train_df,val_df = load_data(path,device)
    ...
    y_train = torch.tensor(train_df['generated'].values,dtype = torch.long)
    x_val = encode(val_df['text'],stoi,block_size)
    y_val = torch.tensor(val_df['generated'].values,dtype = torch.long)
